...
L'iniezione in \textit{batch} e in continuo del \textit{foamer} è attuata sulla linea di mandata del campo San Marco (\figref{fig:mandata}). Inizialmente l'iniezione \textit{in-batch} è stata pensata in una linea ausiliaria appartenente all'impianto di trattamento in quiescenza di San Marco. L'ipotesi è rigettata poiché il \textit{bypass} è collocato a troppa distanza dalla linea di mandata. Lunghe distanze all'interno dell'area possono tradursi in maggiori perdite di carico, inibendo in parte l'azione di "shock" provocata dal \textit{batch}. Inoltre si è ritenuto che il tratto iniziale potesse essere stato interessato da acqua stagnante (il tratto è completamente orizzontale), condizione sfavorevole all'azione ottimale del \textit{foamer}.\\
...
Una volta completata la depressurizzazione si è attuato il \textit{batch} in linea (\figref{fig:test-batch-immissione}). Nella condotta sono stati introdotti 40 litri di acqua ma solamente 60 litri di schiumogeno: il prodotto infatti tracimava dal punto di iniezione ed è stato impossibile continuare l'applicazione.\\
...
\caption{Applicazione di schiumogeno \textit{in-batch}: immissione e dettaglio della PCV.}
...
Dopo un'attesa di 2 ore e 10 minuti dall'applicazione del \textit{batch}, sono confluiti in centrale circa 7 m\ap{3} (con una portata stimata di circa 80 m\ap{3}/h) di acqua mista a schiuma (\figref{fig:test-arrivocentrale}) e residui solidi di colore scuro, associati all'attività di corrosione interna della condotta.
\begin{figure}[htbp]
    \centering
...
\item attenuamento dell'\textit{hold-up} in linea con lo spiazzamento dell'85\% (m\ap{3}) di acqua in condotta;
